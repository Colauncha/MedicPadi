...
export default function ProductList() {
  ...
  const categories = [
    { name: "Antibiotics", count: 50, desc: "Antibiotics products" },
    { name: "Pain Reliver", count: 50, desc: "Pain reliever products" },
    { name: "Supplements", count: 50, desc: "Supplement products" },
    { name: "Diabetes Care", count: 50, desc: "Diabetes products" },
  ];

  // Calculated Stats
  const totalProductsCount = products.length * 20; // Simulated scale
  const lowStockCount = products.filter((p) => p.status === "Low stock").length * 20;
  const outOfStockCount = products.filter((p) => p.status === "Out of stock").length * 20;
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-8 pb-10">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-semibold text-[#150D5E]">Product List</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#F4F3F8] rounded-2xl p-5 flex items-center justify-between border border-[#eef0f6] relative">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-[#150D5E] text-2xl font-bold mt-0.5">1,000</h3>
              </div>
            </div>
          ...
          <div className="bg-[#F4F3F8] rounded-2xl p-5 flex items-center justify-between border border-[#eef0f6] relative">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-gray-500 text-xs font-semibold">Low stock items</p>
                <h3 className="text-[#150D5E] text-2xl font-bold mt-0.5">200</h3>
              </div>
            </div>
            <button className="text-gray-400 hover:text-[#150D5E] p-1.5 rounded-lg">
              <MoreVertical className="w-5 h-5" />
          ...
          <div className="bg-[#F4F3F8] rounded-2xl p-5 flex items-center justify-between border border-[#eef0f6] relative">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-gray-500 text-xs font-semibold">Out of stock</p>
                <h3 className="text-[#150D5E] text-2xl font-bold mt-0.5">68</h3>
              </div>
            </div>
            <button className="text-gray-400 hover:text-[#150D5E] p-1.5 rounded-lg">
              <MoreVertical className="w-5 h-5" />
        ...
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {categories.map((cat, idx) => (
              ...
              >
                <div>
                  <span className="text-[#6F6BD6] text-xs font-bold uppercase tracking-wider">{cat.name}</span>
                  <h4 className="text-[#150D5E] text-2xl font-bold my-1">{cat.count}</h4>
                ...
                <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm relative shrink-0">
                  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 text-[#6F6BD6]" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-4 opacity-95">
            {categories.map((cat, idx) => (
              ...
              >
                <div>
                  <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">{cat.name}</span>
                  <h4 className="text-gray-400 text-2xl font-bold my-1">{cat.count}</h4>
                  <p className="text-gray-400 text-xs">{cat.desc}</p>
                </div>
                <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm relative opacity-60">
                  <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 text-gray-400" stroke="currentColor" strokeWidth="2">
                    <rect x="7" y="8" width="10" height="13" rx="2" />
                    <rect x="9" y="3" width="6" height="5" rx="1" />
                  </svg>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-4 opacity-40">
            {categories.map((cat, idx) => (
              ...
              >
                <span className="text-gray-400 text-xs font-bold">{cat.name}</span>
        ...
        <div className="bg-white border border-[#eef0f6] rounded-2xl p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <h2 className="text-2xl font-bold text-[#150D5E]">{selectedCategory}</h2>
              ...
              <tbody>
                {filteredProducts.length > 0 ? (
                  filteredProducts.map((prod, i) => (
                    ...
                    >
                      <td className="py-4 px-6 font-semibold">{prod.id}</td>
                      <td className="py-4 px-6 font-medium text-gray-800">
                        {prod.name} <span className="text-gray-400 text-xs font-normal">({prod.dosage})</span>
                      </td>
                      <td className="py-4 px-6 text-gray-600">{prod.quantity} units</td>
                      <td className="py-4 px-6 text-gray-500">{prod.expiryDate}</td>
                      ...
                      <td className="py-4 px-6 font-bold text-[#150D5E]">{prod.price}</td>
                      <td className="py-4 px-6">
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-gray-100 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddProduct} className="p-6 flex flex-col gap-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setNewProduct({ ...newProduct, quantity: qty, status });
                    }}
                    className="w-full px-4 h-11 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#150D5E]/20 focus:border-[#150D5E] text-sm"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Auto-Status</label>
